...
pipeline = PipeLine().set_initiator(role="guest", party_id=party_no).set_roles(guest=party_no, host=host_no)
...
reader_0.get_party_instance(role="guest", party_id=party_no).component_param(table=guest_train_data)
reader_0.get_party_instance(role="host", party_id=host_no).component_param(table=host_train_data)
...
data_transform_0.get_party_instance(role="host", party_id=host_no).component_param(with_label=False)
